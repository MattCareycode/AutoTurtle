...
def main():
    ...
    #turtle1_teleport = rospy.ServiceProxy('turtle1/teleport_absolute', teleport_absolute )
    ...
    move_cmd.linear.x = int(raw_input("Enter an angular velocity: ")) # Modify this value to change the Turtle's speed

        # Turn at 0 radians/s
    move_cmd.angular.z = int(raw_input("Enter an Angular Velocity: "))
   # pose = rospy.wait_for_message('turtle1/pose', Pose, update_pose)
    ...
    teleport = rospy.ServiceProxy('turtle1/teleport', TeleportAbsolute)
    ...
    while True:
        ...
        if abs(move_cmd.angular.z) * (now - last) >= 2*math.pi:
            #teleport_absolute(pose.x, pose.y, pose.z)
